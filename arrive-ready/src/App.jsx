...
function generatePlan({ homeTZ, destTZ, chronotype }) {
  ...
  for (let i=-2; i<=recoveryDays; i++) {
    ...
    if (i<0) {
      ...
      hydration={water:"20–32 oz every 4–5 hours",electrolytes:"Start electrolytes 24h before departure",avoid:["Alcohol — disrupts sleep","Excess caffeine after mid-afternoon"],note:"Pre-load hydration now. Pair water with electrolytes for better absorption."};
      food={eat:["High-protein breakfasts & lunches","Complex carbs at dinner","High-water foods: watermelon, cucumber","Tart cherry juice before bed"],avoid:["Heavy fatty meals at night","Refined carbs during the day","Large portions"],note:"Protein activates your active-phase chemistry. Carbs at dinner trigger wind-down."};
    }
    if (i===0) {
      tips.push("Set all devices to destination time zone as you board");
      tips.push("Skip alcohol and minimize caffeine on the plane");
      tips.push(goingEast?"Try to sleep during destination nighttime hours":"Stay awake if it is daytime at your destination when you land");
      hydration={water:"8 oz every hour in-flight — cabin air is extremely dry",electrolytes:"Add electrolyte drops to your water bottle",avoid:["Alcohol — diuretic, worsens jet lag","Coffee unless aligned with destination wake time","Sugary sodas"],note:"You can lose 1-2 liters of fluid on a long flight just through breathing."};
      food={eat:["Light high-protein snacks (nuts, cheese, eggs)","Hydrating foods: watermelon, cucumber","Small meals — heavy meals worsen sleep","Eat on destination schedule where possible"],avoid:["Airline alcohol","Heavy fried meals","Large refined carbs if you need to stay alert"],note:"Eat lightly before destination bedtime, then have a protein breakfast when destination morning arrives."};
    }
    if (i>0 && i<=2) {
      ...
      hydration={water:"8 oz per hour while awake",electrolytes:"Continue electrolytes for 2–3 days post-arrival",avoid:["Alcohol especially in the evening","Caffeine after your cutoff time"],note:"Dehydration slows circadian adjustment. Keep a water bottle visible."};
      food={eat:["High-protein breakfast at local time","Fermented foods (yogurt, kefir, kimchi)","Tart cherry juice before local bedtime","Bananas, oats, quinoa for steady energy"],avoid:["Heavy meals within 2–3 hours of bedtime","Skipping meals","High sugar foods"],note:"A strong local breakfast is your #1 food tool for resetting your clock."};
    }
    if (i>2) {
      tips.push("Maintain consistent sleep and wake times — nearly adjusted");
      tips.push("Continue morning light to reinforce your new circadian rhythm");
      hydration={water:"Return to normal (~8 cups / 64 oz)",electrolytes:"Normal diet should suffice",avoid:["Late-night caffeine","Excess alcohol"],note:"Keep meal and sleep times consistent."};
    ...
    <div style={{ minHeight: "100vh", background: sky.bg, color: "#e8eaf6", fontFamily: F.body, position: "relative", transition: "background 3s ease" }}>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <div style={{ borderBottom: "1px solid rgba(255,255,255,0.09)", padding: "20px 28px 16px", display: "flex", alignItems: "center", gap: "16px", background: "rgba(4,6,28,0.75)", backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)" }}>
          <div onClick={() => setLogo(l => l === "plane" ? "sunrise" : "plane")} style={{ cursor: "pointer", flexShrink: 0, userSelect: "none" }} title="Tap to switch logo style">
          ...
          {step === 1 && <button onClick={() => { setStep(0); setPlan(null); }} style={{ ...pill(false, sky.glow), marginLeft: "auto" }}>← New Plan</button>}
        </div>

        {step === 0 && (
          <div style={{ maxWidth: "580px", margin: "0 auto", padding: "40px 24px" }}>
            <p style={{ color: "rgba(255,255,255,0.65)", fontSize: "14px", lineHeight: "1.9", marginBottom: "36px", fontFamily: F.body }}>
            ...
            <p style={{ fontSize: "11px", color: sky.glow, marginTop: "8px", textAlign: "center", opacity: 0.55, fontFamily: F.body }}>✨ Tap the logo above to switch between logo styles</p>
          </div>
        )}
        {step === 1 && plan && (
          <div style={{ maxWidth: "720px", margin: "0 auto", padding: "28px 20px" }}>
                  ...
                  <div style={{ padding: "20px 24px" }}>
                    ...
                    {activeTab === "schedule" && (
                      ...
                      <div>
                        ...
                        <div style={mc("#38bdf8")}><div style={{ fontSize: "11px", color: "#38bdf8", fontFamily: F.body, fontWeight: 800, marginBottom: "8px" }}>⚡ Electrolytes</div><div style={{ fontSize: "13px", color: "rgba(255,255,255,0.72)", lineHeight: "1.65", fontFamily: F.body }}>{day.hydration.electrolytes}</div><div style={{ marginTop: "6px", fontSize: "11px", color: "rgba(255,255,255,0.38)", fontFamily: F.body }}>Key minerals: sodium, potassium, magnesium</div></div>
                        <div style={mc("#f87171")}><div style={{ fontSize: "11px", color: "#f87171", fontFamily: F.body, fontWeight: 800, marginBottom: "8px" }}>🚫 Avoid</div>{day.hydration.avoid.map((item, idx) => (<div key={idx} style={{ display: "flex", gap: "8px", marginBottom: "7px", fontSize: "13px", color: "rgba(255,255,255,0.68)", fontFamily: F.body }}><span style={{ color: "#f87171", flexShrink: 0 }}>✕</span>{item}</div>))}</div>
                    ...
                    {activeTab === "food" && day.food && (
                      <div>
                        <div style={mc("#4dd9ac")}><div style={{ fontSize: "11px", color: "#4dd9ac", fontFamily: F.body, fontWeight: 800, marginBottom: "8px" }}>✅ Eat & Drink</div>{day.food.eat.map((item, idx) => (<div key={idx} style={{ display: "flex", gap: "8px", marginBottom: "7px", fontSize: "13px", color: "rgba(255,255,255,0.72)", lineHeight: "1.5", fontFamily: F.body }}><span style={{ color: "#4dd9ac", flexShrink: 0 }}>→</span>{item}</div>))}</div>
                        <div style={mc("#f87171")}><div style={{ fontSize: "11px", color: "#f87171", fontFamily: F.body, fontWeight: 800, marginBottom: "8px" }}>🚫 Avoid</div>{day.food.avoid.map((item, idx) => (<div key={idx} style={{ display: "flex", gap: "8px", marginBottom: "6px", fontSize: "13px", color: "rgba(255,255,255,0.68)", fontFamily: F.body }}><span style={{ color: "#f87171", flexShrink: 0 }}>✕</span>{item}</div>))}</div>
                    ...
                    {activeTab === "tips" && (
                      <ul style={{ margin: 0, padding: 0, listStyle: "none" }}>
                        {day.tips.map((tip, i) => (
                          <li key={i} style={{ display: "flex", gap: "12px", marginBottom: "14px", fontSize: "14px", color: "rgba(255,255,255,0.78)", lineHeight: "1.7", fontFamily: F.body }}>
                            <span style={{ color: pc, flexShrink: 0, fontSize: "16px" }}>→</span>{tip}
                          </li>
                        ))}
                      </ul>
                    )}
                    {activeTab === "extras" && (() => {
                      const extras = [
                        { icon: "🏃", title: "Exercise Timing", color: "#fb923c", items: day.phase==="pre"?["Light morning exercise reinforces your wake-time cues","Avoid intense workouts within 3 hours of target bedtime","Short outdoor walk combines light + movement — double benefit"]:day.phase==="travel"?["Walk the aisle every 1–2 hours to improve circulation","Stretch in your seat — ankle circles, shoulder rolls","Avoid intense exercise on travel day"]:["Morning exercise in local sunlight is one of the strongest circadian anchors","Light aerobic activity (walking, easy jog) is best for days 1–2","Avoid exercise within 2–3 hours of target bedtime"], note: "Morning outdoor movement compounds the light benefit and boosts alertness." },
                        { icon: "🌡️", title: "Temperature Therapy", color: "#38bdf8", items: day.phase==="pre"?["Keep bedroom cool 65–68°F (18–20°C)","Warm bath 1–2 hours before bed triggers the temperature-drop response"]:day.phase==="travel"?["Use a blanket if sleeping, remove layers if staying alert","A cool damp cloth on face counters drowsiness"]:["Set hotel thermostat to 65–68°F for sleep","Brief cold shower in the morning replicates the natural cortisol spike","Warm bath 60–90 min before bedtime aids sleep onset"], note: "Cool = sleep mode. Warm = wake mode. Use temperature strategically." },
                        { icon: "😌", title: "Stress & Mental Reset", color: "#b48ef7", items: day.phase==="pre"?["Reduce pre-trip stress — cortisol disrupts sleep","Avoid late-night screens; blue light suppresses melatonin for 1–3 hours","Pack familiar scents — sensory cues improve sleep in new places"]:day.phase==="travel"?["Noise-canceling headphones — sound is the #1 cause of poor in-flight sleep","Eye mask + neck pillow for sleep","Box breathing (4 in, hold 4, out 4, hold 4) helps you fall asleep"]:["Commit to local time — avoid checking home time zone","5–10 min deep breathing before bed helps override the wrong-timezone feeling","Familiar scents (lavender, your usual lotion) cue sleep in unfamiliar rooms"], note: "Stress hormones actively fight your sleep signals. Managing them is as important as managing light." },
                          ...
                          {extras.map(({ icon, title, color, items, note }) => (
                            <div key={title} style={mc(color)}>
                              <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
                                ...
                                <div style={{ fontSize: "11px", color, fontFamily: F.body, fontWeight: 800 }}>{title}</div>
                              </div>
            ...
            <div style={{ marginTop: "16px", padding: "14px 20px", background: "rgba(8,12,45,0.6)", backdropFilter: "blur(14px)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "14px", fontSize: "12px", color: "rgba(255,255,255,0.42)", lineHeight: "1.7", fontFamily: F.body }}>
              <strong style={{ color: "rgba(255,255,255,0.6)" }}>Science note:</strong> Light exposure is the primary driver of circadian clock resets. Seek bright outdoor light at the times shown and avoid it before sleep. Melatonin (0.5–3 mg) works best at low doses timed to your new bedtime.
            </div>
